...
int main(){ 
	...
	for(temp=Tmin;temp<Tmax+dT;temp+=dT){
...
//Read input informations
	...
	ReadInput >> Tmin;
	ReadInput >> Tmax;
	cout << "Temperature range = ["<<Tmin<<";"<<Tmax<<"] with step "<< dT <<endl << endl;
...
//Select randomly a particle (for C++ syntax, 0 <= o <= nspin-1)
		...
		if(metro==1){	 //Metropolis
			...
			double a=1./(1.+exp(-beta*(energy_new-energy_old)));
			accepted++;
			if(r>a) s[o]=-s[o];
		}
	}
...
void Averages(int iblk){	//Print results for current block
	...
	if(iblk==nblk){
		...
		Ene.close();
		Heat.open("output.heatcp.0",ios::app);
